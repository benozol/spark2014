...
package body Gnat2Why.Error_Messages is
   ...
   function Decide_Cntexmp_Verdict
     (Small_Step : SPARK_RAC.Result;
      ...
      VC         : VC_Id;
      Info       : VC_Info)
      return Cntexmp_Verdict is
   begin
      case Small_Step.Res_Kind is
         when Res_Failure =>
            if
              VC_Id (Small_Step.Res_VC_Id) = VC and then
               ...
               return
                 (Verdict_Category => Bad_Counterexample,
                  Verdict_Reason   => To_Unbounded_String
                    ("it triggers a failure at a different check " &
                       "during normal RAC, namely for " &
                       To_String (Small_Step.Res_VC_Kind, Small_Step.Res_Node)
                     & " and not for " & To_String (Info.Kind, Info.Node)));
            end if;

         when Res_Stuck   =>
            return
              (Verdict_Category => Bad_Counterexample,
               Verdict_Reason   => Small_Step.Res_Reason);
            ...
            case Giant_Step.Res_Kind is
               ...
               when Res_Normal  =>
                  return
                    (Verdict_Category => Bad_Counterexample,
                     Verdict_Reason   => To_Unbounded_String
                       ("it doesn't trigger any failure during RAC"));
               when Res_Stuck   =>
                  return
                    (Verdict_Category => Bad_Counterexample,
                     Verdict_Reason   =>
                        "the giant-step RAC " & Giant_Step.Res_Reason);
               when Res_Incomplete =>
                  return
            ...
            case Giant_Step.Res_Kind is
               ...
               when Res_Normal  =>
                  return
                    (Verdict_Category => Incomplete,
                     Verdict_Reason   =>
                        "the normal RAC " & Small_Step.Res_Reason);
               when Res_Stuck   =>
                  return
                    (Verdict_Category => Bad_Counterexample,
                     Verdict_Reason   =>
   ...
   procedure Parse_Why3_Results (Fn : String; Timing : in out Time_Token) is
      ...
      procedure Handle_Result (V : JSON_Value; SD_Id : Session_Dir_Base_ID) is
         ...
         begin
            ...
            if Cntexample_File_Maps.Is_Empty (Cntexmp) then
               ...
               Res := SPARK_RAC.RAC_Execute
                 (E, Cntexmp, Fuel => 1_000_000, Stack_Height => 450);
               --  ??? SPARK_RAC counts during execution the stacked calls in
               --  the interpreted program and terminates as incomplete when
               --  the stack height is exceeded. But we cannot really know how
               --  many calls are in the interpreter between the calls in the
               --  interpreted program, 450 just seems to work. Should we
               --  instead just catch a stack overflow in the interpreter
               --  program?
            end if;
      ...
      begin
         ...
         if Gnat2Why_Args.Check_Counterexamples then
            ...
            exception
               when E : RAC_Unexpected_Error =>
                  Verdict :=
                    (Verdict_Category => Not_Checked,
                     Verdict_Reason   =>
                        To_Unbounded_String
                           ("RAC unexpected error: " &
                          Ada.Exceptions.Exception_Message (E)));
            end;
         else
            Verdict :=
         ...
         if SPARK_RAC.Do_RAC_Info then
            Write_Str
              ("VERDICT: " & To_String (Verdict) &
                 ":" &
               (if Verdict.Verdict_Category in Not_Checked | Incomplete then
                       To_String (Verdict.Verdict_Reason) else "") &
                 ":" & Result_Kind'Image (Small_Step_Res.Res_Kind) &
                 ":" &
               (if Small_Step_Res.Res_Kind in
                      Res_Incomplete | Res_Not_Executed
                  then To_String (Small_Step_Res.Res_Reason) else "") &
                 ":" & Result_Kind'Image (Rec.Giant_Step_Res.Res_Kind) &
                 ":" &
               (if Rec.Giant_Step_Res.Res_Kind in
                      Res_Incomplete | Res_Not_Executed
                  then To_String (Rec.Giant_Step_Res.Res_Reason) else "") &
                 ":");
