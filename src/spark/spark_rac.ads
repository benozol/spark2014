...
package SPARK_RAC is
   ...
   --  Raised when something unforseen happens, but not a program error
   ...
   --  zero. If Stack_Height is non-zero the execution terminates as incomplete
   --  when the stack of calls to procedures or functions grows higher than
   --  this number.
   --  Raises RAC_Unexpected_Error when something unforseen happens
   ...
   --  Values for a one-dimensional array. Keys may be integers or
   --  indices for enum values.
