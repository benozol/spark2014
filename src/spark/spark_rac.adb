...
package body SPARK_RAC is
   ...
   --  Make an record value with fields F
   ...
   --  Shortcut to convert an Uint to an Big_Integer
   ...
   is (Integer_Value (I, Safe_Retysp (Etype (N)), N));
   --  Construct an integer value after checking against type bounds or
   ...
   function Last_Literal (Etyp : Entity_Id) return Entity_Id;
   --  Return the last literal of an enum entity;

   function To_String
     (Fst, Lst : Big_Integer; M : Values_Map.Map; O : Value_Access)
      return String;
   ...
   package Attributes is new Ada.Containers.Hashed_Maps
     (Key_Type        => Name_Id,
      ...
      Hash            => Name_Hash,
      Equivalent_Keys => "=");
   --  Attributes (e.g. X'A for X) in a binding map names to values
   ...
   procedure Set_Value
     (S : in out Scopes.Map; E : N_Defining_Identifier_Id; V : Value_Access);
   ...
   function Find_Binding
     (E : N_Entity_Id; Ctx : in out Context) return Binding;
   ...
   function Import
     (V : Cntexmp_Value; Ty : Entity_Id; N : Node_Id) return Value
     with Pre => Can_Get (N);
   ...
   procedure Get_Integer_Type_Bounds
     (Ty : Entity_Id; Fst, Lst : out Big_Integer) with
   ...
   function First_Array_Component (Ty : Entity_Id) return Entity_Id;
   --  Returns the type of the first array component, and fails for arrays with
   ...
   --  Get the index type and component type, and the bounds of a one-
   --  dimensional, constrained array.
   ...
   function RAC_Expr
     (N : N_Subexpr_Id; Ctx : in out Context; Ty0 : Entity_Id := Empty)
      return Value;
   --  Evaluate node N to a value

   function RAC_Expr_LHS
     (N : N_Subexpr_Id; Ctx : in out Context) return Value_Access;
   ...
   function RAC_Call_Builtin (E : Entity_Id; Sc : Scopes.Map) return Opt_Value;
   ...
   function Param_Scope
     (E : Entity_Id; Args : List_Id; Ctx : in out Context) return Scopes.Map;
   ...
   Do_RAC_Info_Env  : constant Boolean :=
                 Ada.Environment_Variables.Value ("GNAT2WHY_RAC_INFO", "off")
                    = "on";
   --  Enable RAC_Info by environment variable GNAT2WHY_RAC_INFO

   Do_RAC_Trace : constant Boolean :=
                 Ada.Environment_Variables.Value ("GNAT2WHY_RAC_TRACE", "off")
                    = "on";
   ...
   begin
      ...
      for C1 in M1.Iterate loop
         ...
         if
            not Values_Map.Has_Element (C2) or else M1 (C1).all /= M2 (C2).all
         then
            return False;
         end if;
      end loop;
   ...
   begin
      if
        Has_Discriminants (Ty) or else
        Has_Predicates (Ty) or else
        Has_Invariants (Ty) or else
        Is_Class_Wide_Type (Ty) or else
        Has_Dynamic_Predicate_Aspect (Ty)
      then
         RAC_Unsupported ("Unsupported type", Ty);
      end if;
   ...
   begin
      ...
      --  ??? Do we have to distinguish checks for lower bound and upper bound?
   ...
   begin
      if Nkind (E) = N_Character_Literal then
         return Character'Image
           (Get_Character (UI_To_CC (Uint (Char_Literal_Value (E)))));
         --  Get_Unqualified_Decoded_Name_String (Chars (E));
         --  return Char_To_String_Representation
         --    (Name_Buffer (Name_Buffer'First + 1));
   ...
   procedure Get_Array_Info
     (Ty                : Node_Id;
      ...
      Fst, Lst          : out Big_Integer)
   is
      Parent_Ty : constant Node_Id := Parent (Safe_Retysp (Ty));
      Type_Def  : Node_Id;
   begin
      if Nkind (Parent_Ty) /= N_Full_Type_Declaration then
         --  E.g., QB01-015__counterexamples/bytesads.ads:11,
         --  where Nkind (Parent_Ty) = N_Component_Declaration.
         --  ??? How to retrieve the type definition in general?
         RAC_Incomplete ("Parent type not full type declaration");
      end if;
      Type_Def := Type_Definition (Parent (Safe_Retysp (Ty)));
      Index_Ty := First_Array_Component (Type_Def);
      Comp_Ty :=
        Safe_Retysp
          (Etype (Subtype_Indication (Component_Definition (Type_Def))));
      ...
      begin
         ...
         begin
            for Elt of Elts loop
               --  ??? Is there a helper for trim(nodeid)?
   ...
   ---------------------------
   -- First_Array_Component --
   ---------------------------

   function First_Array_Component (Ty : Entity_Id) return Entity_Id is
      Res : Node_Id;
   begin
      if Nkind (Ty) /= N_Constrained_Array_Definition then
         RAC_Unsupported ("Unconstrained array", Ty);
      end if;
      Res := First (Discrete_Subtype_Definitions (Ty));
      if Present (Next (Res)) then
         RAC_Unsupported ("Multi-dimensional array", Ty);
      end if;
      if Nkind (Res) not in N_Has_Entity then
         RAC_Unsupported ("Array component not entity", Ty);
      end if;
      return Retysp (Entity (Res));
   end First_Array_Component;
   ...
   begin
      ...
      if Is_Integer_Type (Ty) then
         ...
         begin
            Get_Array_Info (Ty, Index_Ty, Comp_Ty, Fst, Lst);
            pragma Assert (Is_Integer_Type (Index_Ty) or else
                           Is_Enumeration_Type (Index_Ty));
            ...
            for C in V.Array_Indices.Iterate loop
               ...
               begin
                  ...
                  if
                  not (Fst <= To_Big_Integer (Key) and then
                       To_Big_Integer (Key) <= Lst)
                  then
   ...
   is
      ...
      Etyp       : Entity_Id;
   begin
      --  First, get the bounds of the loop
      case Nkind (Def) is

         when N_Range =>
            --  for Id in Low .. High loop Statements
            if not Is_Integer_Type (Etype (Def)) then
               RAC_Unsupported
                 ("Iterate_Loop_Param_Spec loop range not integer", Def);
            end if;
            Etyp  := Etype (Def);
            Curr :=
              Value_Enum_Integer (RAC_Expr (Low_Bound (Def), Ctx));
            Stop :=
              Value_Enum_Integer (RAC_Expr (High_Bound (Def), Ctx));

         when N_Subtype_Indication =>
            --  for Id in Type range Low High loop Stmts
            declare
               R : constant Node_Id := Range_Expression (Constraint (Def));
            begin
               Etyp := Etype (R);
               Curr :=
                 Value_Enum_Integer (RAC_Expr (Low_Bound (R), Ctx));
               Stop :=
                 Value_Enum_Integer (RAC_Expr (High_Bound (R), Ctx));
            end;

         when N_Identifier =>
            --  for Id in Identifier
            Etyp := Etype (Def);
            if Is_Character_Type (Etyp) then
               Curr := To_Big_Integer (Character'Pos (Character'First));
               Stop := To_Big_Integer (Character'Pos (Character'Last));
            elsif Is_Boolean_Type (Etyp) then
               Curr := To_Big_Integer (Boolean'Pos (Boolean'First));
               Stop := To_Big_Integer (Boolean'Pos (Boolean'Last));
            elsif Is_Integer_Type (Etyp) then
               Get_Integer_Type_Bounds (Etyp, Curr, Stop);
            elsif Is_Enumeration_Type (Etyp) then
               Curr := To_Big_Integer
                 (Enum_Entity_To_Integer (First_Literal (Etyp)));
               Stop := To_Big_Integer
                 (Enum_Entity_To_Integer (Last_Literal (Etyp)));
            else
               RAC_Unsupported ("Loop definition identifier", Def);
            end if;
      ...
      begin
         while Test (Curr, Stop) loop
            ...
            if Is_Integer_Type (Etyp) then
               Val := Integer_Value (Curr, Etyp, Empty);
            elsif Is_Enumeration_Type (Etyp) then
               Val := Enum_Value (UI_From_String (To_String (Curr)), Etyp);
            end if;
   ...
   function Last_Literal (Etyp : Entity_Id) return Entity_Id is
      Res  : Entity_Id;
      Next : Entity_Id := First_Literal (Etyp);
   begin
      loop
         Res := Next;
         Next := Next_Literal (Res);
         exit when Next = Empty;
      end loop;
      return Res;
   end Last_Literal;
   ...
   begin
      while Present (Arg) loop
         ...
         if
           Is_Scalar_Type (Etype (Arg)) or else Ekind (Par) = E_In_Parameter
         then
            B := (Val => new Value'(Copy (RAC_Expr (Arg, Ctx))), others => <>);
         else
            pragma Assert
              ((Ekind (Par) in E_In_Out_Parameter | E_Out_Parameter)
               and then Nkind (Arg) = N_Identifier);
            B := Find_Binding (Entity (Arg), Ctx);
         end if;
   ...
   begin
      ...
      if Is_Ghost_Entity (E) then
         RAC_Incomplete
           ("Cannot run ghost subprogram " & Get_Name_String (Chars (E)));
      end if;
   ...
   function RAC_Call_Builtin
     (E : Entity_Id; Sc : Scopes.Map) return Opt_Value is
   begin
      if -- Ada.Text_IO.Put_Line
        Get_Name_String (Chars (E)) = "put_line" and then
        Present (Scope (E)) and then
        Get_Name_String (Chars (Scope (E))) = "text_io" and then
        Present (Sinfo.Nodes.Scope (Scope (Sinfo.Nodes.Scope (E)))) and then
            ...
            begin
               if Present (Expression (Decl)) then
                  ...
                  if Nkind (Obj_Def) not in N_Has_Entity then
                     RAC_Unsupported ("RAC_Decl with type not entity", Decl);
                  end if;
                  V := Default_Value (Entity (Obj_Def));
               end if;
               Set_Value
   ...
   is
      ...
      begin
         --  ??? Can we find more globals that will be used in the program E?
   ...
   is
      ...
      function RAC_Aggregate return Value is
         ...
         Has_Exprs : constant Boolean :=
                       Nkind (N) = N_Aggregate and then
                         Present (Expressions (N));
         Fst, Lst  : Big_Integer;
         Res       : Value;
      begin
            ...
            if Has_Exprs then
               ...
               declare
                  Ix  : Big_Integer := Value_Enum_Integer
                    (RAC_Expr
                       (Low_Bound (Aggregate_Bounds (N)), Ctx));
                  Hig : constant Big_Integer := Value_Enum_Integer
                    (RAC_Expr
                       (High_Bound (Aggregate_Bounds (N)), Ctx));
               ...
               while Present (As) loop
                  if
                    Nkind (As) = N_Iterated_Component_Association and then
                    Present (Defining_Identifier (As))
                  then
                     RAC_Unsupported
      ...
      begin
         case N_Binary_Op (Nkind (N)) is
            ...
            when N_Multiplying_Operator =>
               if
                 Nkind (N) in N_Op_Divide | N_Op_Mod | N_Op_Rem
      ...
      begin
         ...
         if
           Right_Op = Empty or else not Is_Integer_Type (Etype (Right_Op))
         then
   ...
   function RAC_Expr_LHS
     (N : N_Subexpr_Id; Ctx : in out Context) return Value_Access is
   begin
   ...
   begin
      ...
      pragma Assert (not Has_Variable_Input (E));
